...
final class ProfileDropdown extends ClickableWidget implements NanoPaintable {
    ...
    ProfileDropdown(int x, int y, int width, List<String> entries, int selectedIndex, IntConsumer selected) {
        super(x, y, width, 16, Text.literal("Profiles"));
        this.entries = List.copyOf(entries);
        this.selectedIndex = entries.isEmpty() ? -1 : MathHelper.clamp(selectedIndex, 0, entries.size() - 1);
    ...
    void renderOverlayText(DrawContext context) {
        // Rendered by NanoVG in the final overlay pass.
    }

    private void drawEntry(DrawContext context, int y, int index) {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontal, double vertical) {
        ...
        scrollOffset = MathHelper.clamp(scrollOffset - (int) Math.signum(vertical), 0, maximum);
        return true;
    }
