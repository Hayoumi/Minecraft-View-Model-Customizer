...
public final class NanoVGRenderer implements AutoCloseable {
    ...
    public void render(Consumer<Canvas> draw) {
        ...
        try {
            ...
            float pixelRatio = (float) client.getWindow().getScaleFactor();
    ...
    private void bind(Framebuffer target) {
        if (!(target.getColorAttachment() instanceof GlTexture color)) {
            throw new IllegalStateException("Minecraft framebuffer is not OpenGL-backed");
        }
        if (framebuffer == 0) framebuffer = GL30.glGenFramebuffers();
        GL30.glBindFramebuffer(GL30.GL_FRAMEBUFFER, framebuffer);
        GL32.glFramebufferTexture(GL30.GL_FRAMEBUFFER, GL30.GL_COLOR_ATTACHMENT0, color.getGlId(), 0);
        ...
        GL11.glViewport(0, 0, target.textureWidth, target.textureHeight);
        if (!loggedReady) {
            LOGGER.info("NanoVG attached to Minecraft framebuffer {} ({}x{})", framebuffer, target.textureWidth, target.textureHeight);
            loggedReady = true;
        }
    }
